...
<div class="flex flex-col h-screen p-4 gap-4 text-sm font-sans">
	<header
		class="relative p-4 rounded-xl border border-white/5 bg-white/5 backdrop-blur-md shadow-2xl overflow-hidden"
	>
		<div
		...
		<div class="relative z-10 flex items-center gap-3 pointer-events-none">
			<div class="flex items-center gap-3 mr-4 pl-2">
				<div
					class="w-2.5 h-2.5 rounded-full bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.8)]"
				></div>
			...
			<div class="pointer-events-auto flex items-center gap-2 ml-2 pl-4 border-l border-white/5">
				<button
					...
					class="p-2 hover:bg-white/10 rounded-lg text-zinc-500 hover:text-white transition-colors"
				>
					<svg
				...
				>
					<svg
						...
						stroke-width="3"
						stroke-linecap="round"
						stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2" /></svg
					>
				</button>
				<button
					aria-label="Close"
					on:click={() => appWindow?.close()}
					class="p-2 hover:bg-rose-500/20 rounded-lg text-zinc-500 hover:text-rose-400 transition-colors"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="14"
	...
	<div class="flex flex-1 gap-4 overflow-hidden">
		<div
			class="w-1/2 flex flex-col rounded-xl border border-white/5 bg-white/5 backdrop-blur-md overflow-hidden shadow-2xl"
		>
			<div class="flex border-b border-white/5 bg-black/20">
				<button
					class="px-6 py-3 text-xs font-bold tracking-wide transition-colors {activeTab === 'body'
						? 'text-rose-400 border-b-2 border-rose-500 bg-white/5'
						: 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}"
					on:click={() => (activeTab = "body")}>JSON BODY</button
				>
				<button
					class="px-6 py-3 text-xs font-bold tracking-wide transition-colors {activeTab === 'script'
						? 'text-rose-400 border-b-2 border-rose-500 bg-white/5'
						: 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}"
					on:click={() => (activeTab = "script")}>PRE-REQUEST SCRIPT</button
				>
			</div>

			<div class="flex-1 relative text-xs group">
				<button
					on:click={_handleClear}
					class="absolute top-3 right-5 z-20 text-zinc-500 hover:text-rose-400 transition-colors p-1 rounded hover:bg-white/5"
					title="Clear content"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
		...
		>
			<div
				class="px-5 py-3 border-b border-white/5 text-zinc-500 text-[10px] uppercase tracking-widest font-bold flex justify-between items-center bg-black/20"
			>
				<span>Response</span>
				{#if response && !error && !loading}
					<span
						class="text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20"
						>200 OK</span
					>
				{/if}
			</div>
			...
			<div class="flex-1 overflow-auto bg-transparent relative custom-scrollbar">
				{#if loading}
					<div class="flex flex-col items-center justify-center h-full text-zinc-600 gap-4">
						<div
							class="animate-spin rounded-full h-8 w-8 border-2 border-zinc-800 border-t-rose-500"
						></div>
						<span class="text-xs tracking-widest animate-pulse font-bold"
							>CONTACTING SATELLITE...</span
						>
					</div>
				{:else if error}
					<div
						class="text-rose-300 p-4 bg-rose-950/30 rounded border border-rose-500/20 font-mono text-xs"
					>
						<strong class="block mb-2 text-rose-500">REQUEST FAILED</strong>{error}
					</div>
				{:else if response}
					<div class="h-full text-xs">
						<CodeMirror
							...
							theme={oneDark}
							readonly={true}
							extensions={[transparentTheme, keymap.of([...defaultKeymap])]}
							class="h-full"
				...
				{:else}
					<div
						class="flex flex-col items-center justify-center h-full text-zinc-700 gap-2 opacity-50"
					>
